...
@interface InterfaceController() {
	...
	BOOL	launchFromGlance;					// グランスから起動された
	CLLocationCoordinate2D glanceLocation;		// グランスから起動した地点
}
...
- (void)awakeWithContext:(id)context {
	...
	launchFromGlance = NO;
	glanceLocation.latitude = glanceLocation.longitude = NAN;
}
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations {
	...
	if(launchFromGlance==YES) {
		if(isnan(glanceLocation.latitude)) {
			CLLocation *location = [locations firstObject];
			glanceLocation = location.coordinate;
...
- (void)displayCenterAnnotation {
	...
	// グランスから起動された場合は、その地点を赤ピンでマークする
	if(!isnan(glanceLocation.latitude)) {
		[_mapView addAnnotation:glanceLocation withPinColor:WKInterfaceMapPinColorRed];
	}
}
#endif
...
#pragma mark - Glance information

- (void)handleUserActivity:(NSDictionary *)userInfo {
	
	NSLog(@"param=%@", userInfo[@"param"]);

	NSString *paramStr = userInfo[@"param"];
	
	if([paramStr isEqualToString:@"launchFromGlance"]) {
		launchFromGlance = YES;
	}
}
